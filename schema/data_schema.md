...
```
            ...
            │                    │                        │
    ┌───────┴──────┐    ┌────────┴────────┐    ┌──────────┴────────┐
    │ dim_category │    │  dim_shop       │    │   fact_product    │◄─── BẢNG TRUNG TÂM
    └──────────────┘    └─────────────────┘    └────────┬──────────┘
                                                        │
                                ┌───────────────────────┼──────────────────┐
                                │                       │                  │
                    ┌───────────┴──────┐     ┌──────────┴──────┐  ┌────────┴─────────────┐
                    │   fact_review    │     │ bridge_product_ │  │  (Ghi chú: không cần │
                    │                  │     │   promotion     │  │   bảng orders riêng; │
                    └──────────────────┘     └─────────────────┘  │   dùng sold_count &  │
                                                                  │   shipping trong     │
                                                                  │   fact_product)      │
                                                                  └──────────────────────┘
...
|-----|------|----------|-------|
| `platform_id` | `str` | ✗ | **PK** — slug viết thường: `shopee`, `lazada`, `tiki`, `sendo` |
| `platform_name` | `str` | ✗ | Tên hiển thị: `Shopee`, `Lazada`, ... |
| `base_url` | `str` | ✓ | URL gốc của sàn |
...
| Cột | Kiểu | Nullable | Mô tả |
|-----|------|----------|-------|
| `category_id` | `str` | ✗ | **PK** — `{platform_id}_{id_gốc_trên_sàn}` |
| `platform_id` | `str` | ✗ | **FK** → `dim_platform.platform_id` |
| `category_name` | `str` | ✗ | Tên danh mục (tiếng Việt) |
| `parent_category_id` | `str` | ✓ | FK tự tham chiếu → danh mục cha (`null` nếu là gốc) |
| `level` | `int` | ✗ | Cấp độ: `1`=gốc, `2`=cấp 2, `3`=cấp 3 |
| `category_url` | `str` | ✓ | URL trang danh mục |
...
| Cột | Kiểu | Nullable | Mô tả |
|-----|------|----------|-------|
| `shop_id` | `str` | ✗ | **PK** — `{platform_id}_{id_gốc}` |
| `platform_id` | `str` | ✗ | **FK** → `dim_platform.platform_id` |
| `shop_name` | `str` | ✗ | Tên gian hàng |
| `shop_type` | `str` | ✗ | `mall` · `official` · `individual` · `unknown` |
| `is_mall` | `bool` | ✗ | `True` nếu là Shopee Mall / LazMall / Tiki Trading |
| `shop_rating` | `float` | ✓ | Điểm đánh giá gian hàng (0.0–5.0) |
| `follower_count` | `int` | ✓ | Số lượt theo dõi |
| `response_rate` | `float` | ✓ | Tỷ lệ phản hồi chat (0.0–1.0) |
| `response_time_hours` | `float` | ✓ | Thời gian phản hồi trung bình (giờ) |
| `prep_time_hours` | `float` | ✓ | Thời gian chuẩn bị hàng trung bình (giờ) |
| `location` | `str` | ✓ | Tỉnh/Thành phố đặt kho |
| `total_products` | `int` | ✓ | Số sản phẩm đang bán |
| `shop_url` | `str` | ✓ | URL trang gian hàng |
| `crawled_at` | `str` | ✗ | ISO 8601: `2026-03-23T14:00:00+07:00` |
...
| Cột | Kiểu | Nullable | Mô tả |
|-----|------|----------|-------|
| `product_id` | `str` | ✗ | **PK** — `{platform_id}_{id_gốc}` |
| `platform_id` | `str` | ✗ | **FK** → `dim_platform` |
| `category_id` | `str` | ✗ | **FK** → `dim_category` |
| `shop_id` | `str` | ✗ | **FK** → `dim_shop` |
...
| `product_url` | `str` | ✗ | URL trang sản phẩm (dùng truy vết) |

### 5.3 Giá cả

| Cột | Kiểu | Nullable | Mô tả |
|-----|------|----------|-------|
| `price_current` | `float` | ✗ | Giá hiện tại (VNĐ) — đã bỏ ký tự `₫`, `.`, `,` |
| `price_original` | `float` | ✓ | Giá gốc trước KM (`null` nếu không có) |
| `discount_percent` | `float` | ✗ | Phần trăm giảm giá (0.0–100.0); `0.0` nếu không giảm |
| `price_ends_with_9` | `bool` | ✗ | `True` nếu đuôi giá kết thúc bằng `9` (ví dụ: 199k, 299k)|
| `price_bucket` | `str` | ✓ | Phân khúc giá: `<100k` · `100k-500k` · `500k-1M` · `1M-5M` · `>5M` |
...
| Cột | Kiểu | Nullable | Mô tả |
|-----|------|----------|-------|
| `sold_count` | `int` | ✓ | Số lượng đã bán (proxy cho tỷ lệ chuyển đổi) |
| `stock` | `int` | ✓ | Số lượng còn tồn kho (`-1` nếu không hiển thị) |
| `rating` | `float` | ✓ | Điểm đánh giá TB (0.0–5.0) |
...
| Cột | Kiểu | Nullable | Mô tả |
|-----|------|----------|-------|
| `image_count` | `int` | ✓ | Số ảnh trên trang sản phẩm |
| `has_video` | `bool` | ✓ | Có video sản phẩm hay không |
| `review_with_image_count` | `int` | ✓ | Số review có kèm ảnh |
| `five_star_with_image_count` | `int` | ✓ | Số review 5 sao có kèm ảnh |

### 5.6 Vận chuyển

| Cột | Kiểu | Nullable | Mô tả |
|-----|------|----------|-------|
| `shipping_fee` | `float` | ✓ | Phí vận chuyển (VNĐ); `0.0` nếu miễn phí |
| `is_freeship` | `bool` | ✗ | `True` nếu được miễn phí vận chuyển |
| `estimated_delivery_days` | `float` | ✓ | Số ngày giao hàng dự kiến |

### 5.7 Khuyến mãi

| Cột | Kiểu | Nullable | Mô tả |
|-----|------|----------|-------|
| `has_freeship_xtra_label` | `bool` | ✗ | Có nhãn "Freeship Xtra" hay không |
| `has_coinback_label` | `bool` | ✗ | Có nhãn "Hoàn xu" / "Shopee Xu" hay không |
| `has_voucher_label` | `bool` | ✗ | Có voucher giảm giá từ shop hay không |
| `promotion_label_count` | `int` | ✗ | Tổng số nhãn KM đang hiển thị |

### 5.8 Metadata

| Cột | Kiểu | Nullable | Mô tả |
|-----|------|----------|-------|
| `crawled_at` | `str` | ✗ | Thời điểm cào — ISO 8601 |
| `crawled_by` | `str` | ✗ | Tên thành viên cào: `thinh` · `tuan` · `y` · `the_anh` · `duong` |
...
| Cột | Kiểu | Nullable | Mô tả |
|-----|------|----------|-------|
| `review_id` | `str` | ✗ | **PK** — `{platform_id}_{id_gốc}` |
| `product_id` | `str` | ✗ | **FK** → `fact_product.product_id` |
| `rating` | `int` | ✗ | Số sao (1–5) |
| `has_image` | `bool` | ✗ | Review có kèm ảnh|
| `has_video` | `bool` | ✗ | Review có kèm video |
| `comment_length` | `int` | ✓ | Độ dài comment (số ký tự); `0` nếu không có text |
| `review_date` | `str` | ✓ | Ngày đánh giá — `YYYY-MM-DD` |
| `crawled_at` | `str` | ✗ | ISO 8601 |

> **Lưu ý**: Bảng này **không lưu nội dung comment gốc** để tránh file quá nặng.
> Nếu cần NLP/sentiment → lưu thêm cột `comment_text` (tuỳ chọn).
...
|-----|------|----------|-------|
| `product_id` | `str` | ✗ | **FK** → `fact_product.product_id` |
| `promotion_label` | `str` | ✗ | Tên nhãn: `freeship_xtra` · `coinback` · `flash_sale` · `voucher` · `mall_sale` |
| `crawled_at` | `str` | ✗ | ISO 8601 |
...
| **DƯƠNG 2** | Response rate × shop_rating → trust | `dim_shop`, `fact_product` | `response_rate`, `shop_rating`, `sold_count` |
...
| **Tiền tệ** | Số thực (VNĐ), không có ký tự `₫` hay dấu `.` phân cách nghìn |
...
- [ ] Encoding `utf-8-sig`
- [ ] Không thiếu cột **bắt buộc** (Nullable = ✗)
- [ ] `price_current` là `float`, không ký tự `₫` hay `.` ngàn
